...
void Shader::compile(std::string shaderPath)
{
	...
	if (shaderPath.ends_with("glsl")) {
		...
		if (needToCompile) {
			...
			fs::path logFile = shaderRootPath / "log.txt";
			std::string command = compilerPath.string() + " -T" + phase + " -E main -spirv -fspv-preserve-bindings -fspv-preserve-interface -fspv-target-env=vulkan1.3 -Zpr -Od -Fo " + outputShader.string() + " " + hlslShader.string() + " > " + logFile.string();
