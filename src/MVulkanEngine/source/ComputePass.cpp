...
void ComputePass::Create(std::shared_ptr<ComputeShaderModule> shader,
...
{
    setShader(shader);
    CreatePipeline(allocator, storageBuffers, seperateImageViews, storageImageViews, samplers, accelerationStructures);
}
...
void ComputePass::CreatePipeline(MVulkanDescriptorSetAllocator& allocator,
    ...
    for (auto binding = 0; binding < m_bindings.size(); binding++) {
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
        {
            ...
            m_storageBuffer[m_bindings[binding].binding.binding - m_cbvCount] = storageBuffers[m_bindings[binding].binding.binding - m_cbvCount];
            break;
        }
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
        {
    ...
    m_pipeline.Create(
        m_device.GetDevice(), 
        m_shader->GetComputeShader().GetShaderModule(), 
        m_descriptorLayout.Get(), m_shader->GetEntryPoint());
