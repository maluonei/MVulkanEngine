...
{
    auto primsInfos = scene->m_primInfos;
    auto numMeshes = primsInfos.size();

    m_buildInfos = std::vector<VkAccelerationStructureBuildGeometryInfoKHR>(numMeshes);
    m_rangeInfos = std::vector<VkAccelerationStructureBuildRangeInfoKHR>(numMeshes);
    m_blas = std::vector<VkAccelerationStructureKHR>(numMeshes);
    m_asBuffers = std::vector<Buffer>(numMeshes);
    m_scrathBuffers = std::vector<Buffer>(numMeshes);
    
    std::vector<BLASBuildInfo>                                  blasBuildInfos(numMeshes);
    
    auto rayTracingCommandList = Singleton<MVulkanEngine>::instance().GetRaytracingCommandList();
    
    for(auto index = 0; index < numMeshes; index++)
    {
        auto mesh = scene->GetMesh(scene->m_primInfos[index][0].mesh_id);
    ...
    //build tlas
    CreateTLAS(scene, m_asBuffers, m_tlasBuildInfo);
...
void MVulkanRaytracing::CreateTLAS(
    const std::shared_ptr<Scene>& scene, 
    std::vector<Buffer> asBuffers, 
    TLASBuildInfo& tlasBuildInfo)
{
    auto numInstances = scene->GetTotalPrimInfos();
    auto primInfos = scene->m_primInfos;
    ...
    for (auto i = 0; i < numMeshs; i++) {
        auto numMeshInstances = primInfos[i].size();
        for (auto j = 0; j < numMeshInstances; j++) {
            instances[instanceIndex].transform = GLMToVkTransformMatrixKHR(primInfos[i][j].transform);
            instances[instanceIndex].instanceCustomIndex = instanceIndex;
            instances[instanceIndex].mask = 0xFF;
            instances[instanceIndex].instanceShaderBindingTableRecordOffset = 0;
            instances[instanceIndex].flags = VK_GEOMETRY_INSTANCE_TRIANGLE_FACING_CULL_DISABLE_BIT_KHR;

            VkAccelerationStructureDeviceAddressInfoKHR addressInfo{ VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_DEVICE_ADDRESS_INFO_KHR };
            addressInfo.accelerationStructure = m_blas[i];
            vkGetAccelerationStructureDeviceAddressKHR(m_device.GetDevice(), &addressInfo);

            instances[instanceIndex].accelerationStructureReference = vkGetAccelerationStructureDeviceAddressKHR(m_device.GetDevice(), &addressInfo);
            instanceIndex++;
        }
    }
