    ...
    std::string entryPoint)
{
    ...
    VK_CHECK_RESULT(vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline));
...
void MVulkanGraphicsPipeline::Create(
    VkDevice device, MVulkanPilelineCreateInfo info,
    VkShaderModule taskShaderModule, VkShaderModule meshShaderModule, VkShaderModule fragShaderModule,
    VkRenderPass renderPass, VkDescriptorSetLayout layout, uint32_t numAttachments,
    std::string taskEntryPoint, std::string meshEntryPoint, std::string fragEntryPoint)
{
    m_device = device;
    m_info = info;
    ...
    if (taskShaderModule != nullptr) {
        ...
        taskShaderStageInfo.pName = taskEntryPoint.c_str();
    }
    ...
    meshShaderStageInfo.pName = meshEntryPoint.c_str();
...
void MVulkanGraphicsPipeline::Create(VkDevice device,
    MVulkanPilelineCreateInfo info,
    VkShaderModule vertShaderModule,
    VkShaderModule geomShaderModule,
    VkShaderModule fragShaderModule,
    ...
    std::string fragEntryPoint)
{
    ...
    //colorBlendAttachment.colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
    //colorBlendAttachment.blendEnable = VK_FALSE;
    auto numAttachments = m_info.colorAttachmentFormats.size();
    std::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments(numAttachments);
    for (auto i = 0; i < numAttachments; i++)
    {
        VkPipelineColorBlendAttachmentState colorBlendAttachment{};
    ...
    pipelineRenderingInfo.colorAttachmentCount = numAttachments;
    pipelineRenderingInfo.pColorAttachmentFormats = info.colorAttachmentFormats.data();
    pipelineRenderingInfo.depthAttachmentFormat = info.depthAttachmentFormats;
    pipelineRenderingInfo.stencilAttachmentFormat = info.stencilAttachmentFormat;  // 没有 stencil
...
void MVulkanGraphicsPipeline::Create(VkDevice device, MVulkanPilelineCreateInfo info, VkShaderModule vertShaderModule, VkShaderModule geomShaderModule, VkShaderModule fragShaderModule, PipelineVertexInputStateInfo vertexStateInfo, std::vector<VkDescriptorSetLayout> layouts, std::string vertEntryPoint, std::string geomEntryPoint, std::string fragEntryPoint)
{
    m_device = device;
    m_info = info;
