...
[[vk::binding(7, 0)]] StructuredBuffer<MaterialBuffer> materials : register(t1034);

[[vk::binding(8, 0)]] Texture2D<float4> textures[1024] : register(t5);

[[vk::binding(9, 0)]] SamplerState linearSampler : register(s0);

[[vk::binding(10, 0)]] RaytracingAccelerationStructure Tlas : register(t1032);
...
PSOutput main(PSInput input)
{
    ...
    float halfHeight = camera.zNear * tan(camera.fovY * 0.5f);
    float halfWidth = screen.WindowRes.x * halfHeight * (1.f / screen.WindowRes.y);

    float3 forward = normalize(camera.cameraDir);
    ...
    float3 offsetz = forward * camera.zNear;
    ...
    PathState pathState;
    {
        ...
        if(RayTracingClosestHit(ray, pathState)){
            ...
            output.MetallicAndRoughness = float4((pathState.t / 20.f) * float3(1.f, 1.f, 1.f), 1.f);
          }
        else{
