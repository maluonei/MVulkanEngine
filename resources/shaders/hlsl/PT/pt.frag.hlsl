...
[[vk::binding(8, 0)]]Texture2D<float4> gBufferNormal : register(t0);
[[vk::binding(9, 0)]]Texture2D<float4> gBufferPosition : register(t1);
[[vk::binding(10, 0)]]Texture2D<float4> gAlbedo : register(t2);
[[vk::binding(11, 0)]]Texture2D<float4> gMetallicAndRoughness : register(t3);
...
float3 PathTracing(float3 fragPosition, float3 fragNormal, int maxBounces){
    ...
    for(int step=0;step<maxBounces;step++){
        
        //sample rays
        for(int i = 0;i<ubo1.lightNum;i++){
            Light light = ubo1.lights[i];
            float3 L = light.direction;

            RayDesc ray;
            ray.TMin = 0.f;
            ray.TMax = MAX_RAY_DISTANCE; 
            ray.Origin = position + normal * 1e-5; 
            ray.Direction = -L;

            bool hasHit = RayTracingAnyHit(ray);  
            float lightPdf = 1.f;
            float brdf = (1.f / PI) * abs(dot(ray.Direction, normal));

            radiance += hasHit * brdf * beta * light.color * light.intensity / lightPdf;
        }
